...
class _TemperaturePageState extends State<TemperaturePage> {
  final DatabaseReference _dbRef = FirebaseDatabase.instance.ref('serre/capteurs/temperature');
  late SensorData _currentData;
  List<SensorData> _historyData = [];
  bool _isLoading = true;
  double _minThreshold = 15;
  double _maxThreshold = 35;
  double _warningThreshold = 28;
  ...
  Future<void> _loadThresholds() async {
    try {
      final thresholdsRef = FirebaseDatabase.instance.ref('serre/parametres/seuils_alertes/temperature');
      final snapshot = await thresholdsRef.get();
      
      if (snapshot.exists) {
        final data = snapshot.value as Map<dynamic, dynamic>;
        setState(() {
          _warningThreshold = (data['warning'] ?? 28).toDouble();
        });
      }
      ...
      if (actuelleSnapshot.exists) {
        ...
        setState(() {
          _minThreshold = (actuelleData['seuil_min'] ?? 15).toDouble();
          _maxThreshold = (actuelleData['seuil_max'] ?? 35).toDouble();
        });
      }
    } catch (e) {
  ...
  Widget _buildThresholdInfo() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Text('Seuils de température', style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 10),
            Row(
  ...
  Widget _buildThresholdChip(String label, double value, Color color) {
    return Column(
      children: [
        ...
        Chip(
          label: Text('${value.toStringAsFixed(1)}°C'),
            ...
            Container(
              ...
              child: LineChart(
                LineChartData(
                  ...
                  minY: _minThreshold - 5,
                  maxY: _maxThreshold + 5,
          ...
          children: [
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  leading: Icon(Icons.thermostat, color: _getValueColor(data.value)),
                  title: Text('${data.value.toStringAsFixed(1)}°C'),
